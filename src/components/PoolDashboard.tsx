...
import { Search, ChevronRight, Activity, Wallet, PieChart, RefreshCw } from 'lucide-react';
...
function TreasuryTokenRow({ symbol, address }: { symbol: string; address: string }) {
    ...
    <tr className="hover:bg-white/5 transition-colors group">
      <td className="px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <p className="font-bold text-white group-hover:text-amber-500 transition-colors">{symbol}</p>
            <p className="text-[10px] text-slate-500">Token</p>
          </div>
        </div>
      </td>
      <td className="px-6 py-4 text-right">
        <p className="text-sm font-bold text-white">{balance} {symbol}</p>
      </td>
      <td className="px-6 py-4 text-right">
    ...
    <div className="w-full max-w-5xl mx-auto space-y-6">
      ...
      {view === 'VAULTS' && (
        <>
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Pool di Liquidità</h1>
              <p className="text-slate-400">
                Scopri e investi nelle pool più performanti su BSC con tecnologia Arbitrage Inception.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <RefreshCw size={13} className={pricesLoading ? 'animate-spin text-amber-500' : ''} />
                {lastUpdated
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              >
                ...
                <p className="text-xs text-slate-500 mb-5">Commissioni {pool.fee}</p>

                <div className="flex justify-between items-end">
                  <div>
                    <p className="text-xs text-slate-500 mb-1">APY</p>
                    <p className={`text-2xl font-bold ${pool.apy > 100 ? 'text-green-400' : 'text-slate-200'}`}>
                      {pool.apy >= 1_000_000
                        ? (pool.apy / 1_000_000).toLocaleString(undefined, { maximumFractionDigits: 2 }) + 'M'
                        : pool.apy >= 1000
                        ? pool.apy.toLocaleString(undefined, { maximumFractionDigits: 1 })
                        : pool.apy.toLocaleString(undefined, { maximumFractionDigits: 2 })}%
                    </p>
                  </div>
      ...
      {view === 'PORTFOLIO' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-[#131A2A] border border-[#1E293B] rounded-2xl p-6">
              <h3 className="text-sm text-slate-400 mb-2">Rendimento Giornaliero</h3>
          ...
          {!wallet || positions.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              {positions.map((p) => {
                ...
                const livePool = pools.find((pl) => pl.id === p.poolId) ?? p.pool;
                ...
                const inRange = maxPrice !== Infinity && minPrice > 0
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className="flex -space-x-2">
                        <img
                          src={livePool?.token0Logo}
                          alt={livePool?.token0}
                          className="w-10 h-10 rounded-full bg-slate-800 border-2 border-[#131A2A] shadow-sm"
                        />
                        <img
                          src={livePool?.token1Logo}
                          alt={livePool?.token1}
                          className="w-10 h-10 rounded-full bg-slate-700 border-2 border-[#131A2A] shadow-sm"
                        />
                      </div>
                      <div>
                        ...
                        {minPrice > 0 && maxPrice !== Infinity && currentPrice > 0 && (
                          <div className="mt-2 flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-1 text-[9px] text-slate-500 font-mono">
                              <span className="text-slate-400">{minPrice.toLocaleString()}</span>
                              <ChevronRight size={8} />
                              <span className="text-slate-400">{maxPrice.toLocaleString()}</span>
                            </div>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex flex-row md:flex-col justify-between items-center md:items-end w-full md:w-auto">
                      <div className="text-left md:text-right">
                        <p className="text-xs text-slate-500 mb-1">Depositato</p>
                        <p className="text-lg font-bold text-white">
                          ${p.amountUSD.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                        </p>
                      </div>
                      <div className="text-right">
      ...
      {view === 'TREASURY' && (
        <div className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Protocol Treasury</h1>
              <p className="text-slate-400">
                Monitoraggio in tempo reale dei guadagni e delle riserve del protocollo Arbitrage Inception.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="bg-[#131A2A] border border-[#1E293B] rounded-2xl p-6">
              <h3 className="text-sm text-slate-400 mb-2 font-medium">Tesoro Monitorato</h3>
              <p className="text-lg font-mono text-white break-all">{DEV_FEE_ADDRESS}</p>
              <p className="text-[10px] text-slate-500 mt-2">Indirizzo ufficiale del protocollo</p>
            </div>
            <div className="bg-[#131A2A] border border-[#1E293B] rounded-2xl p-6 flex items-center justify-between">
              <div>
                <h3 className="text-sm text-slate-400 mb-2 font-medium">Status</h3>
                <div className="flex items-center gap-2">
                  <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
          ...
          <div className="bg-[#131A2A] border border-[#1E293B] rounded-2xl overflow-hidden">
            <div className="p-6 border-b border-[#1E293B] flex items-center justify-between">
              <h3 className="text-lg font-bold text-white">Composizione Asset Tesoro</h3>
              <span className="text-xs text-slate-500 font-mono">Aggiornato ogni 15s</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-[#1E293B]/50 bg-[#0D111C]/30">
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest">Token</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest text-right">Bilancio</th>
                    <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase tracking-widest text-right">Valore (USD)</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#1E293B]/30">
                  <TreasuryTokenRow symbol="BNB" address="0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE" />
                  <TreasuryTokenRow symbol="USDT" address="0x55d398326f99059fF775485246999027B3197955" />
                  <TreasuryTokenRow symbol="CAKE" address="0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82" />
                  <TreasuryTokenRow symbol="USDC" address="0x8ac76a51cc950d9822d68b83fe1ad97b32cd580d" />
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div className="bg-amber-500/5 border border-amber-500/10 rounded-2xl p-6 text-center">
            ...
            <p className="text-slate-400 text-sm max-w-2xl mx-auto leading-relaxed">
              Le fee generate dalla tecnologia Arbitrage Inception vengono accumulate nel Treasury Wallet. Queste risorse
              sono dedicate alla stabilità del protocollo, al riacquisto di token e ai premi per i fornitori di liquidità
              più fedeli.
            </p>
          </div>
        </div>
