...
import { Settings, ArrowDown, Info, AlertTriangle, ExternalLink, ArrowUp, Wallet, Activity } from 'lucide-react';
...
export default function ZapWidget({ 
          ...
          try {
            ...
            if (count > 0) {
              ...
              const positions = await Promise.all(
                validIds.map(id => positionContract.positions(id).then(res => ({ id, res })).catch(() => null))
              );
  ...
  const handleAction = async () => {
    ...
    setTxHash('');
    setTxModalParams({ isOpen: true, step: 0, message: 'Confirming in Wallet...' });
  ...
  const forceCompleteTransaction = () => {
    ...
    if (tab === 'IN') {
      ...
      if (tokenIn.symbol.includes('BNB') || tokenIn.symbol === 'WBNB') amountUSD *= getPrice('BNB');
    ...
    <div className="bg-[#131A2A] border border-[#1E293B] rounded-[24px] shadow-2xl relative overflow-hidden">
      {txModalParams.isOpen && (
         <div className="absolute inset-0 bg-[#0A0D14]/95 backdrop-blur-sm z-50 flex items-center justify-center flex-col p-6 rounded-[24px]">
            {txModalParams.step < 3 && <div className="w-12 h-12 border-4 border-amber-500 border-t-transparent rounded-full animate-spin mb-4" />}
      ...
      <div className="p-4 space-y-2">
          ...
          <div className="flex justify-between items-center mb-3">
            ...
            {tab === 'IN' && wallet && (
               <div className="flex items-center gap-1.5 text-xs text-slate-400">
                  ...
                  <button onClick={() => setAmount(balanceIn)} className="text-amber-400 hover:text-amber-300 ml-1 font-medium bg-amber-500/10 px-1.5 py-0.5 rounded">MAX</button>
               </div>
            )}
          </div>
            ...
            <div className="flex flex-col gap-3">
                ...
                <div className="mt-2.5 bg-[#0A0D14] p-3 rounded-lg border border-[#1E293B]/60">
                  <div className="flex justify-between items-center mb-2.5 border-b border-[#1E293B]/40 pb-2">
                    ...
                    {detectingPositions ? (
                      <span className="text-[10px] text-amber-400 animate-pulse font-mono">Scansione {scannedCount > 0 ? `${scannedCount} NFT...` : 'Wallet...'}</span>
                    ) : (
                      <div className="flex items-center gap-2">
                         <span className="text-[10px] text-slate-500 font-mono">{displayPositions.length} Trovate</span>
                         <button 
                  ...
                  <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                    {displayPositions.length === 0 ? (
                      <div className="py-6 flex flex-col items-center justify-center text-center px-4">
                        ...
                        <p className="text-[11px] text-slate-500 font-medium">
                          {showAllNfts ? 'Nessun NFT V3 trovato nel wallet.' : 'Nessuna posizione LP attiva per questa coppia.'}
                        </p>
                        ...
                        >
                          {showManualInput ? 'Annulla Inserimento' : 'Inserisci ID NFT Manualmente'}
                        </button>
                      </div>
                    ) : (
                          ...
                          >
                            ...
                            <div className="text-left">
                              <p className="text-xs font-bold text-white flex items-center gap-2">
                                <span className={`w-2 h-2 rounded-full ${isSelected ? 'bg-amber-400 shadow-[0_0_8px_rgba(245,158,11,0.5)]' : (pos.isMatch ? 'bg-green-500/40' : 'bg-red-500/40')}`}></span>
                                NFT ID: #{pos.id}
                                {isSelected && <span className="text-[9px] bg-amber-500 text-white px-1.5 py-0.5 rounded-md uppercase tracking-tighter">Selezionato</span>}
                                {!pos.isMatch && <span className="text-[9px] bg-amber-500/20 text-amber-400 border border-amber-500/30 px-1.5 py-0.5 rounded-md uppercase tracking-tighter">Differente</span>}
                              </p>
                    ...
                    {(showManualInput || (displayPositions.length > 0)) && (
                      <div className="mt-4 pt-4 border-t border-[#1E293B]/40">
                         ...
                         >
                            {showManualInput ? 'Nascondi Manuale' : 'Inserimento Manuale ID NFT'}
                         </button>
                         {showManualInput && (
                            <div className="flex flex-col gap-2 bg-[#0A0D14] p-3 rounded-lg border border-[#1E293B]/60">
                               ...
                               />
                               <p className="text-[9px] text-slate-500">Usa solo se la scansione automatica fallisce.</p>
                            </div>
                         )}
                      </div>
                    )}
        ...
        <div className="bg-[#0D111C] p-4 rounded-xl border border-[#1E293B]/50 hover:border-[#1E293B] transition-colors relative">
          <div className="flex justify-between items-center mb-3">
            <span className="text-sm font-medium text-slate-400">
               {tab === 'IN' ? 'Pool di Destinazione (Ricevi LP)' : 'Ricevi Token'}
            </span>
            <div className="flex items-center gap-3">
               {tab === 'OUT' && wallet && (
                 <div className="flex items-center gap-1.5 text-xs text-slate-400">
             ...
             {loading ? (
                ...
                <div className="space-y-2 p-1 text-left">
                  ...
                  <div className="bg-amber-500/10 border border-amber-500/20 p-2.5 rounded-lg text-[11px] text-amber-300 leading-normal mt-2">
                    <strong className="block mb-0.5 font-bold">💡 Esecuzione On-Chain</strong>
                    Assicurati che il tuo wallet sia connesso su rete BSC. Prelemi e depositi sono reali.
                  </div>
                </div>
             ) : route ? (
                <div className="space-y-3">
                  <div className="flex justify-between items-center text-slate-300">
                     <span className="flex items-center gap-1.5"><Info size={14} className="text-slate-500"/> Stima Gas</span>
                  ...
                  {tab === 'IN' && amount && Number(amount) > 0 && targetPool.apy && (
                     <div className="flex justify-between items-center text-slate-300 pt-2 border-t border-[#1E293B]/50">
                        <span className="text-slate-400 text-xs flex items-center gap-1.5"><Activity size={12}/> Guadagno Giornaliero Stm.</span>
                        <span className="font-mono text-green-400 text-xs">
                          {(() => {
                             const amountUSD = Number(amount) * getPrice(tokenIn.symbol);
                             const dailyReturn = amountUSD * ((1 + targetPool.apy / 100)**(1 / 365) - 1);
                  ...
                  <div className="flex justify-between items-center text-slate-300">
                     ...
                     <span className="text-amber-400 flex items-center gap-1">Arbitrage Inception ZaaS <ExternalLink size={12}/></span>
                  </div>
                </div>
             ) : null}
          </div>
